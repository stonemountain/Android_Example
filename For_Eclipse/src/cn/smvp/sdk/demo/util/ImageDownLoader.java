    ...
    private Bitmap downloadBitmap(String url) {
        ...
        try {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                ...
                myLogger.w("Error " + statusCode +
                        " while retrieving bitmap from " + url);
                return null;
            }
        } catch (IOException exception) {
            getRequest.abort();
            myLogger.w("I/O error while retrieving bitmap from " + url, exception);
        } catch (IllegalStateException e) {
            getRequest.abort();
            myLogger.w("Incorrect URL: " + url);
        } catch (Exception e) {
            getRequest.abort();
            myLogger.w("Error while retrieving bitmap from " + url, e);
        } finally {
